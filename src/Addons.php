...
abstract class Addons
{
    ...
    public $addons_path = '';
    // 插件配置作用域
    protected $configRange = null;
    // 插件信息作用域
    protected $infoRange = null;
    ...
    public function __construct()
    {
        ...
        $config = array_merge(\think\facade\Config::pull('template'), $config);
        $view = new View();
        $this->view = $view->init(
            $config,
            \think\facade\Config::get('app.view_replace_str')
        );
        //$this->view = new View($config, \think\facade\Config::get('app.view_replace_str'));
    ...
    final public function getInfo($name = '')
    {
        ...
        $info = \think\facade\Config::get($name.'info');
        if ($info) {
            return $info;
        }
        $info_file = $this->addons_path . 'info.ini';
        if (is_file($info_file)) {
            $info = \think\facade\Config::parse($info_file, '', $name.'info');
            $info['url'] = addon_url($name);
        }
        \think\facade\Config::set($name.'info', $info);
    ...
    final public function getConfig($name = '')
    {
        ...
        $config = \think\facade\Config::get($name);
        if ($config) {
            return $config;
        }
        ...
        \think\facade\Config::set($name, $config);
        return $config;
    }
    ...
    final public function setInfo($name = '', $value = [])
    {
        ...
        \think\facade\Config::set($name, $info, $this->infoRange);
        return $info;
    }
